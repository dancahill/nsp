...
{
	flush(N->getN()); /* if we die here, we should flush the buffer first */
	switch (sig) {
	case 11:
		printf("Segmentation Violation\r\n");
		if ((N->getN())&&(N->getN()->readptr)) printf("[%s][%.40s]\r\n", N->getN()->tracefn, N->getN()->readptr);
		exit(-1);
	case 13: /* SIGPIPE */
		return;
	default:
...
static void preppath(NesState *N, char *name)
{
	char buf[1024];
	char *p;
	unsigned int j;
	...
	nes_setstr(N->getN(), N->getG(), "_filename", p, -1);
	nes_setstr(N->getN(), N->getG(), "_filepath", buf, -1);
	return;
}
...
int main(int argc, char *argv[])
{
	...
	N->setdebug(0);
#ifdef HAVE_CDB
	neslacdb_register_all(N->getN());
#endif
#ifdef HAVE_CRYPTO
	neslacrypto_register_all(N->getN());
#endif
#ifdef HAVE_DL
	nesladl_register_all(N->getN());
#endif
	neslaext_register_all(N->getN());
#ifdef HAVE_LDAP
	neslaldap_register_all(N->getN());
#endif
#ifdef HAVE_MATH
	neslamath_register_all(N->getN());
#endif
#ifdef HAVE_MYSQL
	neslamysql_register_all(N->getN());
#endif
#ifdef HAVE_ODBC
	neslaodbc_register_all(N->getN());
#endif
#ifdef HAVE_PIPE
	neslapipe_register_all(N->getN());
#endif
#ifdef HAVE_PGSQL
	neslapgsql_register_all(N->getN());
#endif
#ifdef HAVE_REGEX
	neslaregex_register_all(N->getN());
#endif
#ifdef HAVE_SQLITE3
	neslasqlite3_register_all(N->getN());
#endif
#ifdef HAVE_SSH2
	neslassh_register_all(N->getN());
#endif
#ifndef __TURBOC__
#ifndef TINYCC
	neslatcp_register_all(N->getN());
#endif
#endif
#ifdef WIN32
	neslawinapi_register_all(N->getN());
#endif
#ifdef HAVE_ZLIB
	neslazip_register_all(N->getN());
#endif
	/* add env */
	tobj=nes_settable(N->getN(), N->getG(), "_ENV");
	for (i=0;environ[i]!=NULL;i++) {
		...
		*p='\0';
		...
		nes_setstr(N->getN(), tobj, tmpbuf, p, -1);
	}
	/* add args */
	tobj=nes_settable(N->getN(), N->getG(), "_ARGS");
	for (i=0;i<argc;i++) {
		n_ntoa(N->getN(), tmpbuf, i, 10, 0);
		nes_setstr(N->getN(), tobj, tmpbuf, argv[i], -1);
	}
	tobj=nes_settable(N->getN(), N->getG(), "io");
	nes_setcfunc(N->getN(), tobj, "gets", (NES_CFUNC)neslib_io_gets);
	for (i=1;i<argc;i++) {
