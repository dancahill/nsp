...
void cgi_readenv()
{
	...
	obj_t *servobj=nes_settable(N, &N->g, "_SERVER");
	obj_t *cookieobj=nes_settable(N, &N->g, "_COOKIE");
	obj_t *gobj=nes_settable(N, &N->g, "_GET");
	obj_t *pobj=nes_settable(N, &N->g, "_POST");
	obj_t *cobj;
	char boundary[100];
	int blen;
	int clen;
	...
	char filename[255];
	char *QueryString=nes_getstr(N, servobj, "QUERY_STRING");
	char *p1, *p2;
	unsigned int len;
	int i;
	int x;
	...
	if (cobj->val->type==NT_STRING) {
		p1=nes_tostr(N, cobj);
		len=strlen(nes_getstr(N, servobj, "SCRIPT_NAME"));
		if (len<strlen(p1)) {
			nes_setstr(N, servobj, "CGI_URI", p1+len, strlen(p1)-len);
		} else {
			nes_setstr(N, servobj, "CGI_URI", "/", 1);
		}
	}
	p1=QueryString;
	while (*p1!='\0') {
		...
		cobj=nes_setstr(N, gobj, tmpbuf, p1, len);
		if (cobj->val->d.str) {
			URLDecode(cobj->val->d.str); cobj->val->size=strlen(cobj->val->d.str);
		}
		p1+=i;
	}
	if (strcmp(nes_getstr(N, servobj, "REQUEST_METHOD"), "POST")==0) {
		clen=(int)nes_getnum(N, servobj, "CONTENT_LENGTH");
		if (clen>nes_getnum(N, confobj, "max_postsize")) {
		...
		cobj=nes_getobj(N, &N->g, "POSTRAWDATA");
		...
		p1=nes_getstr(N, servobj, "CONTENT_TYPE");
		if (strncmp(p1, "multipart/form-data", strlen("multipart/form-data"))==0) {
			...
			cobj=nes_setstr(N, &N->g, "POSTBOUNDARY", boundary, blen);
			cobj->val->attr|=NST_HIDDEN;
			p1=nes_getstr(N, &N->g, "POSTRAWDATA");
			p2=p1;
			do {
				if (p2-p1>clen) break;
				...
				if (p2[i-1]=='\n') i--; if (p2[i-1]=='\r') i--;
				cobj=nes_setstr(N, pobj, tmpbuf, NULL, 0);
				if (i) {
			...
			while (*p1!='\0') {
				...
				cobj=nes_setstr(N, pobj, tmpbuf, p1, len);
				if (cobj->val->d.str) {
					URLDecode(cobj->val->d.str); cobj->val->size=strlen(cobj->val->d.str);
				}
				p1+=i;
	...
	p1=nes_getstr(N, servobj, "HTTP_COOKIE");
	if (p1) {
		for (;;) {
			...
			nes_setstr(N, cookieobj, tmpbuf, p1, p2-p1);
			if (!strlen(p2)) break;
			p2++;
			while (isspace(*p2)) p2++;
			p1=p2;
...
{
	obj_t *cobj=nes_getobj(N, nes_settable(N, &N->g, "_GET"), query);
...
{
	obj_t *cobj=nes_getobj(N, nes_settable(N, &N->g, "_POST"), query);
...
{
	obj_t *pobj=nes_settable(N, &N->g, "_POST");
	obj_t *cobj=nes_getobj(N, pobj, "ATTACHMENT1");
