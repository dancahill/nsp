...
{
#include <stdio.h>
	char *original="$1$0hs2u/6B$KxBgAAw59fBu/DyHA/Vor/";
	char rpassword[80];
	char *salt="$1$0hs2u/6B$";

	if (argc>1) {
		snprintf(rpassword, sizeof(rpassword)-1, "%s", argv[1]);
	} else {
		snprintf(rpassword, sizeof(rpassword)-1, "visual");
	}
	printf("[%s]\r\n", original);
	printf("[%s]\r\n", md5_crypt(rpassword, salt));
	return 0;
}
*/
