...
static int mysqlConnect(nes_state *N, MYSQL_CONN *conn, char *host, char *user, char *pass, char *db, int port)
{
	...
	if (conn->mysock==NULL) {
		n_warn(N, "mysqlConnect", "MySQL connection error");
		return -1;
	}
	return 0;
}
...
void mysql_murder(nes_state *N, obj_t *cobj)
{
	...
	n_warn(N, "mysql_murder", "reaper is claiming another lost soul");
	if ((cobj->val->type!=NT_CDATA)||(cobj->val->d.str==NULL)||(nc_strcmp(cobj->val->d.str, "mysql-conn")!=0))
		n_error(N, NE_SYNTAX, "mysql_murder", "expected a mysql conn");
...
NES_FUNCTION(neslamysql_query)
{
	...
	if (rc<0) {
		nes_setstr(N, &N->r, "", "mysql connection error", strlen("mysql connection error"));
		n_free(N, (void *)&conn);
		return -1;
	}
