...
extern regexp *_regcomp(nsp_state *N, const char *re);
extern int _regexec(nsp_state *N, regexp *rp, const char *s);
extern void _regsub(nsp_state *N, const regexp *rp, const char *src, char *dst);
extern void _regerror(nsp_state *N, char *message);
...
char *dest;
{
	...
	if (prog == NULL || source == NULL || dest == NULL) {
		_regerror(N, "NULL parameter to regsub");
		return;
	}
	if ((unsigned char)*(prog->program) != MAGIC) {
		_regerror(N, "damaged regexp");
		return;
	}
		...
		if (no < 0) {	/* Ordinary character. */
			...
			*dst++ = c;
			...
			if (*(dst - 1) == '\0') {	/* strncpy hit NUL. */
				_regerror(N, "damaged match string");
				return;
			}
		}
	}
...
* Utility definitions.
*/
#define	FAIL(m)		{ _regerror(N, m); return(NULL); }
...
*/
static char *reg(nsp_state *N, struct comp *cp, int paren, int *flagp);
static char *regbranch(nsp_state *N, struct comp *cp, int *flagp);
static char *regpiece(nsp_state *N, struct comp *cp, int *flagp);
static char *regatom(nsp_state *N, struct comp *cp, int *flagp);
...
* Forwards.
*/
static int regtry(nsp_state *N, struct exec *ep, regexp *rp, char *string);
static int regmatch(nsp_state *N, struct exec *ep, char *prog);
static size_t regrepeat(nsp_state *N, struct exec *ep, char *node);
...
const char *str;
{
	...
	if (prog == NULL || string == NULL) {
		_regerror(N, "NULL argument to regexec");
		return(0);
	}

	/* Check validity of program. */
	if ((unsigned char)*prog->program != MAGIC) {
		_regerror(N, "corrupted regexp");
		return(0);
	}
...
char *prog;
{
	...
	*/
	_regerror(N, "corrupted pointers");
	return(0);
}
...
char *node;
{
	...
	default:		/* Oh dear.  Called inappropriately. */
		_regerror(N, "internal error: bad call of regrepeat");
