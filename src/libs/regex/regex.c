...
extern regexp *regcomp(nes_state *N, const char *re);
extern int regexec(nes_state *N, regexp *rp, const char *s);
extern void regsub(nes_state *N, const regexp *rp, const char *src, char *dst);
extern void regerror(nes_state *N, char *message);
...
char *s;
{
	n_error(N, NE_SYNTAX, "regerror", "%s", s);
/*
#ifdef ERRAVAIL
	error("regexp: %s", s);
#else
...
char *dest;
{
	...
	if (prog == NULL || source == NULL || dest == NULL) {
		regerror(N, "NULL parameter to regsub");
		return;
	}
	if ((unsigned char)*(prog->program) != MAGIC) {
		regerror(N, "damaged regexp");
		return;
	}
		...
		if (no < 0) {	/* Ordinary character. */
			...
			*dst++ = c;
			...
			if (*(dst-1) == '\0') {	/* strncpy hit NUL. */
				regerror(N, "damaged match string");
				return;
			}
		}
	}
 ...
 * Utility definitions.
 */
#define	FAIL(m)		{ regerror(N, m); return(NULL); }
 ...
 */
static char *reg(nes_state *N, struct comp *cp, int paren, int *flagp);
static char *regbranch(nes_state *N, struct comp *cp, int *flagp);
static char *regpiece(nes_state *N, struct comp *cp, int *flagp);
static char *regatom(nes_state *N, struct comp *cp, int *flagp);
 ...
 * Forwards.
 */
static int regtry(nes_state *N, struct exec *ep, regexp *rp, char *string);
static int regmatch(nes_state *N, struct exec *ep, char *prog);
static size_t regrepeat(nes_state *N, struct exec *ep, char *node);
...
const char *str;
{
	...
	if (prog == NULL || string == NULL) {
		regerror(N, "NULL argument to regexec");
		return(0);
	}

	/* Check validity of program. */
	if ((unsigned char)*prog->program != MAGIC) {
		regerror(N, "corrupted regexp");
		return(0);
	}
	...
	if (prog->reganch)
		return(regtry(N, &ex, prog, string));
...
char *string;
{
	...
	if (regmatch(N, ep, prog->program + 1)) {
...
char *prog;
{
	...
	for (scan = prog; scan != NULL; scan = next) {
		...
		case STAR: case PLUS: {
			...
			for (no = regrepeat(N, ep, OPERAND(scan)) + 1; no > min; no--) {
...
char *node;
{
	...
	default:		/* Oh dear.  Called inappropriately. */
		regerror(N, "internal error: bad call of regrepeat");
...
int neslaregex_register_all(nes_state *N)
{
	...
	tobj=nes_settable(N, &N->g, "regex");
	tobj->val->attr|=NST_HIDDEN;
	nes_setcfunc(N, tobj, "match",   (NES_CFUNC)neslaregex_match);
	nes_setcfunc(N, tobj, "replace", (NES_CFUNC)neslaregex_replace);
	return 0;
}
