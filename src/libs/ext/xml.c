...
static void neslaext_xml_trimtree(nes_state *N, obj_t *tobj)
{
		...
		if (*p1=='<') {
			...
			if (*p1=='/') {
				...
				if (nc_isalnum(*p1)||*p1==':'||*p1=='_'||*p1=='-') {
					...
					n_error(N, NE_SYNTAX, __FUNCTION__, "unexpected data");
				}
...
//				printf("end [%s][%s]\n", otag, tagbuf);
				...
				p2=p1;
				while (*p2&&*p2!='>') p2++;
				if (*p2!='>') n_error(N, NE_SYNTAX, __FUNCTION__, "missing >");
				p2++;
				e=p2;
				if (strcmp(otag, tagbuf)==0) break;
//				n_warn(N, __FUNCTION__, "end tag '%s' '%s'", otag, tagbuf);
...
//				continue;
				...
				if (nc_isalnum(*p1)||*p1==':'||*p1=='_'||*p1=='-') {
					...
					for (;;) {
						...
						if (*p1=='=') {
							...
							if (*p1) {
								...
								cobj=nes_setstr(N, aobj, namebuf, tb, p1-tb);
								if ((q)&&(*p1==q)) p1++;
								/* if (N->debug) n_warn(N, __FUNCTION__, "new attr  '%s->%s' = '%s'", cobj->parent->parent->parent->name, cobj->name, nes_tostr(N, cobj)); */
							}
						}
						while (nc_isspace(*p1)) p1++;
						if (*p1=='/') {
					...
					n_error(N, NE_SYNTAX, __FUNCTION__, "unexpected data [%s]", p1);
				}
				while (nc_isspace(*p1)) p1++;
				p2=p1;
				while (*p2&&*p2!='>') {
...
//			nl_flush(N);cobj=nes_setstr(N, &N->l, "debug", b, e-b);printf("tag %d[%s]\n", is_end, nes_tostr(N, cobj));
			b=e;
			if (!is_end) {
				b=neslaext_xml_readsub(N, nobj, b, tagbuf, eb);
			}
		}
...
//			nl_flush(N);cobj=nes_setstr(N, &N->l, "debug", b, e-b);	printf("dat %d[%s]\n", is_dat, nes_tostr(N, cobj));
		}
		b=e;
/*
continue;
...
//			n_warn(N, __FUNCTION__, "x end label '%s' '%s'", tobj->name, nobj->name);
//			if (!strlen(tobj->name)) 
			*eb=1;
			b=e;
//			goto x;
...
//		if ((tobj!=NULL)&&(tobj->name[0]!='@')) {
		...
		if (N->debug) n_warn(N, __FUNCTION__, "... '%s'", tobj->name);
//x:
//		n_warn(N, __FUNCTION__, "eb1=%d", *eb);
		if (!*eb) {
//			n_warn(N, __FUNCTION__, "begin label '%s' '%s'", tobj->name, nobj->name);
//			b=neslaext_xml_readsub(N, nobj, b, eb);
//			n_warn(N, __FUNCTION__, "end label '%s' '%s'", tobj->name, nobj->name);
//			if (*eb) { *eb=0; break; }
//			if (*eb) { *eb=0; }
		}
//		n_warn(N, __FUNCTION__, "eb2=%d", *eb);
//			if (*eb) { *eb=0; }
//		if (*eb) { *eb=0; }
		if (b==NULL) break;
*/
...
NES_FUNCTION(neslaext_xml_read)
{
	...
	neslaext_xml_readsub(N, &tobj, cobj1->val->d.str, "", &eb);
	if (!nes_isnull(cobj2)) trim=nes_tobool(N, cobj2)?1:0;
	if (trim) neslaext_xml_trimtree(N, &tobj);
