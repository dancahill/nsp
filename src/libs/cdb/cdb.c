...
#define __FUNCTION__ __FILE__ ":neslacdb_read()"
	...
	for (i=0;i<256;i++) {
		...
		for (j=0;j<count;j++) {
			...
			len=readi4((nbuf+4));
			cobj=nes_setstr(N, &tobj, namebuf, NULL, 0);
			if (!len) continue;
			cobj->val->size=len;
...
int neslacdb_register_all(nes_state *N)
{
	...
	tobj=nes_settable(N, &N->g, "cdb");
	tobj->val->attr|=NST_HIDDEN;
	nes_setcfunc(N, tobj, "read",  (NES_CFUNC)neslacdb_read);
